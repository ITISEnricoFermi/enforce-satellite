...
var XBee = /** @class */ (function (_super) {
    ...
    XBee.prototype.check = function (string) {
        if (string.indexOf(COMMAND_DELIMETER) !== -1)
            ...
            this.emit("loc", string.split(DELIMETERS.loc)[1]);
        else if (string.indexOf(DELIMETERS.ori) !== -1)
            this.emit("ori", string.split(DELIMETERS.ori)[1]);
        else if (string.indexOf(DELIMETERS.pre) !== -1)
            this.emit("pre", string.split(DELIMETERS.pre)[1]);
        else if (string.indexOf(DELIMETERS.tmp) !== -1)
            this.emit("tmp", string.split(DELIMETERS.tmp)[1]);
        else if (string.indexOf(DELIMETERS.umd) !== -1)
            this.emit("umd", string.split(DELIMETERS.umd)[1]);
        else
     ...
     * @param {any} data
     */
    XBee.prototype.sentLOC = function (data) {
